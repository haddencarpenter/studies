...
## Why ep3 exists (read this first)

Ep3 is organic content for the **methodology-as-trust** layer of the Shumi thesis. Shumi is the composable crypto intelligence primitive for developer workflows; the film series makes the *apparatus* (validation gates, provenance, regime positions) legible — not to teach trading, but to earn integration trust from two audiences:

1. **Developers / latent buyers** evaluating signals + data for the MCP wrapper or gated chat at `shumi.ai/chat`.
2. **Latent strategic acquirers** cultivating awareness of the operating system, not the P&L.

**Viewer CTA:** every episode needs a closing call-to-action pointing somewhere actionable — `shumi.ai/chat`, an MCP install link, or the next study. If a draft has no CTA, flag it before re-publishing.

**Out of scope for the series:** charisma-led broadcasting, "alpha to follow" framing, anything that turns the films into a content treadmill for its own sake. When making judgment calls below — refresh cadence, freshness automation, ep4 scope — bias toward what serves the two audiences above.
...
6. **`20260509-scanner-silent-17-days-pre-ep4.md` — flag this one first.** Regime scanner emitted 0 signals in last 17 days; needs disposition before ep4 publishing time. The other 5 are background canonicalization that can wait.

These are READ-ONLY drafts for now — promote into `pxeodev/docs/canonical-memory.mdx` post-ship in a focused triage pass.

## Ideas to plug in (ranked by leverage)

You mentioned wanting to contribute ideas so we don't keep using stale data. Ranked against the treadmill — pick #1 unless you have a strong reason otherwise, since it's structurally upstream of everything below it:

1. **Decoupled data layer (start here).** Pull all numeric values from a JSON manifest instead of hardcoding into `index.html`. Film code stays static; only the data file changes on refresh. Removes the treadmill at its root and makes every other idea below cheaper to build on top.
2. **Per-scene `data-freshness="YYYY-MM-DD"` attribute.** Once the manifest exists, every scene auto-stamps from its source artifact's timestamp. Visible to viewers via hover or footer — surfaces stale data instead of hiding it.
3. **Refresh script that opens a draft PR.** Automate the 10-step workflow into one command. Far easier once the manifest exists (step 6 becomes a write to JSON, not 6 grep-and-replace edits across HTML).
4. **CI staleness gate** — block merges to main if displayed numbers don't match a fresh re-query. Useful, but a band-aid if the underlying values are still hardcoded. Sequence this *after* the decoupled layer.
5. **Production checklist markdown** — largely falls out for free once the manifest exists (the manifest *is* the checklist).

Or bring your own framing — I'd rather hear your read than impose one. But if you want the fastest path off the treadmill, #1 is the move.
